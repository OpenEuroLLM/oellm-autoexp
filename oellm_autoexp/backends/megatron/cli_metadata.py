...
MEGATRON_ARG_METADATA: Mapping[str, MegatronArgMetadata] = {
    ...
    "allow_new_param_groups_on_load": MegatronArgMetadata(
        arg_type=bool,
        default=False,
        help="When the loaded optimizer checkpoint has no param_group matching a current group (a multiplier enabled mid-run creates a new group), load the new group from the structurally equivalent saved group and keep the current overrides instead of raising.",
        choices=None,
        nargs=0,
        element_type=None,
    ),
    ...
    "diag_consumed_train_samples": MegatronArgMetadata(
        arg_type=int,
        default=None,
        help="Diagnostics only: after the checkpoint load, position the training dataloader at this consumed-sample count instead of the checkpoint's own, so a probe can run the weights of iteration X on the batches of iteration Y (Y * global batch size). Never use for real training.",
        choices=None,
        nargs=None,
        element_type=None,
    ),
    ...
    "diag_swap_keys": MegatronArgMetadata(
        arg_type=str,
        default=None,
        help="Comma-separated regexes matched against the checkpoint keys (global layer numbering, e.g. ^output_layer[.]weight or ^decoder[.]layers[.]5[6-9][.]) selecting the tensors to reload from --diag-swap-checkpoint.",
        choices=None,
        nargs=None,
        element_type=None,
    ),
    ...
    "embedding_wd_mult": MegatronArgMetadata(
        arg_type=float,
        default=1.0,
        help="Multiplier on --weight-decay for the word-embedding matrix (2-D param whose name contains word_embeddings). Default 1.0 = decay like every other weight. 0.0 excludes the embeddings from weight decay as OLMo 2/3 and Levanter/Marin do (decay erodes the rows of rare tokens). The untied output layer is not affected.",
        choices=None,
        nargs=None,
        element_type=None,
    ),
    ...
    "log_output_logsumexp": MegatronArgMetadata(
        arg_type=bool,
        default=False,
        help="If True, log the LM head's mean log-normalizer squared, mean(logsumexp(logits)**2), even when ``output_z_loss_coeff`` is None. That statistic is normally a by-product of the z-loss, so switching the z-loss off also removes the only view of the quantity it and ``final_logit_softcapping`` both exist to control. Forward-only: the log-normalizer is detached, so no gradient is added.",
        choices=None,
        nargs=0,
        element_type=None,
    ),
    ...
    "output_z_loss_fp32_grad_accum": MegatronArgMetadata(
        arg_type=bool,
        default=False,
        help="If True, sum the z-loss and cross-entropy logit gradients in fp32 before rounding the total to the logits dtype. Only meaningful when ``output_z_loss_coeff`` is set and the logits are in reduced precision. The fused ``native`` cross-entropy otherwise rounds the CE gradient to bf16 FIRST and adds the z-loss gradient to it in bf16; since that gradient is ``2 * coeff * logZ`` times the CE gradient (~1.6e-3 at coeff=1e-4, logZ~8) it lands at or below bf16's 2**-9 unit roundoff and is largely lost. Costs one fp32 buffer the size of the logits shard for the duration of the loss backward.",
        choices=None,
        nargs=0,
        element_type=None,
    ),
    ...
    "te_debug_config": MegatronArgMetadata(
        arg_type=str,
        default=None,
        help="nvdlfw_inspect feature YAML applied to every Transformer Engine module (LogTensorStats, LogFp8TensorStats, ...). Writes PER-RANK statistics files, so use it only on small probes (<= 16 nodes). See megatron/training/te_debug.py.",
        choices=None,
        nargs=None,
        element_type=None,
    ),
...
MEGATRON_ACTION_SPECS: Mapping[str, MegatronActionSpec] = {
    ...
    "log_output_logsumexp": MegatronActionSpec(
        option_strings=("--log-output-logsumexp",),
        action_type="store_true",
        nargs=0,
        const=True,
        default=False,
    ...
    "output_z_loss_fp32_grad_accum": MegatronActionSpec(
        option_strings=("--output-z-loss-fp32-grad-accum",),
        action_type="store_true",
        nargs=0,
        const=True,
        default=False,
