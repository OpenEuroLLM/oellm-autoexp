...
class MegatronConfig(ConfigInterface):
    ...
    # (i.e. ``tensor_model_parallel_size * gtp_weight_remat_size``). This is the user-facing
    # knob: it must be ``>= tensor_model_parallel_size`` and divisible by it. When None it
    # defaults to ``tensor_model_parallel_size`` (no GTP sharding). It is the source of truth
    # and implies ``gtp_weight_remat_size = tensor_parallel_num_weight_shards //
    # tensor_model_parallel_size`` (resolved in ``__post_init__``).
    tensor_parallel_num_weight_shards: int | None | None = None

    # Configuring backend option of pipeline parallel communication (e.g., nccl, ucc) If None,
    # the default backend will be used.
    pipeline_model_parallel_comm_backend: Literal["nccl", "ucc"] | None | None = None
    ...
    # communication type.
    hierarchical_context_parallel_sizes: list[int] | None | None = None
    ...
    max_seqlen_per_dp_cp_rank: int | None | None = None
    ...
    expert_tensor_parallel_size: int | None | None = None

    # Total number of shards each expert weight is split into across the expert-tensor-
    # parallel + expert-GTP axes (i.e. ``expert_tensor_parallel_size *
    # expert_gtp_weight_remat_size``). This is the user-facing knob for expert layers: it must
    # be ``>= expert_tensor_parallel_size`` and divisible by it. When None it defaults to
    # ``expert_tensor_parallel_size`` (no expert GTP sharding). It is the source of truth and
    # implies ``expert_gtp_weight_remat_size = expert_tensor_parallel_num_weight_shards //
    # expert_tensor_parallel_size`` (resolved in ``__post_init__``).
    expert_tensor_parallel_num_weight_shards: int | None | None = None
    ...
    # models.
    use_cpu_initialization: bool | None | None = None
    ...
    microbatch_group_size_per_vp_stage: int | None | None = None
    ...
    mtp_num_layers: int | None | None = None
    ...
    # If True, detach MTP head inputs from the main model graph. This prevents MTP loss
    # gradients from flowing back to the main model, only training the MTP heads themselves.
    ...
    mtp_hybrid_override_pattern: str | None | None = None
    ...
    num_attention_heads: int | None | None = None
    ...
    # (TransformerEngine) and inference (mcore dynamic-batching) attention paths. When None,
    # each path selects a version automatically based on what is installed. Pinning is
    # required for batch-invariant mode: the training-side logprob recompute and the inference
    # engine must run the same kernel, since different FlashAttention generations use
    # different tile sizes and softmax accumulation orders and therefore differ bitwise. On
    # the training side this is enforced via TransformerEngine's NVTE_FLASH_ATTN_V2/V3/V4
    # selection environment variables.
    flash_attention_version: Literal[2, 3, 4] | None | None = None
    ...
    # provided.
    ffn_hidden_size: int | None | None = None

    # Projection weights dimension in multi-head attention. This is set to hidden_size //
    # num_attention_heads if not provided.
    kv_channels: int | None | None = None
    ...
    activation_func_clamp_value: float | None | None = None
    ...
    experimental_attention_variant: Literal["gated_delta_net", "dsa"] | None | None = None

    # Number of DSA indexer heads.
    dsa_indexer_n_heads: int | None | None = None

    # Dimension per DSA indexer head.
    dsa_indexer_head_dim: int | None | None = None

    # Number of top-k tokens to select in DSA indexer.
    dsa_indexer_topk: int | None | None = None

    # Frequency of DSA indexer top-k computation across layers. A value greater than 1 enables
    ...
    dsa_indexer_loss_coeff: float | None | None = None

    # Whether to use sparse DSA indexer loss. If True, the indexer loss will be computed using
    # the top-k indices.
    dsa_indexer_use_sparse_loss: bool = False

    # Optional fused DSA kernel backend. ``none`` disables fused DSA kernels. Explicit
    # ``tilelang`` or ``cudnn`` enables only that backend. Unsupported DSA layouts continue to
    # use the PyTorch fallback.
    ...
    dsa_indexer_k_norm_epsilon: float | None | None = None
    ...
    embedding_init_method_std: float | None | None = None
    ...
    mup_base_hidden_size: int | None | None = None
    ...
    mup_base_head_dim: float | None | None = None
    ...
    # If set, soft-cap the final output-layer (LM head) logits with ``c * tanh(logits / c)``
    # where ``c`` is this value (e.g. 30.0 as in Gemma 2). Bounds the logits to ``(-c, c)``.
    ...
    # This is the LM-head analog of ``moe_z_loss_coeff`` for the router.
    output_z_loss_coeff: float | None | None = None

    # If True, log the LM head's mean log-normalizer squared, mean(logsumexp(logits)**2), even
    # when ``output_z_loss_coeff`` is None. That statistic is normally a by-product of the
    ...
    # CE gradient to bf16 FIRST and adds the z-loss gradient to it in bf16; since that
    # gradient is ``2 * coeff * logZ`` times the CE gradient (~1.6e-3 at coeff=1e-4, logZ~8)
    # it lands at or below bf16's 2**-9 unit roundoff and is largely lost. Costs one fp32
    # buffer the size of the logits shard for the duration of the loss backward.
    ...
    # If True, submodules may use Transformer Engine's operation fuser API to enable advanced
    # fusions.
    ...
    recompute_granularity: Literal["full", "selective"] | None | None = None
    ...
    # 'uniform' or 'block'.
    recompute_method: Literal["uniform", "block"] | None | None = None
    ...
    recompute_num_layers: int | None | None = None

    # If True, distribute recomputed activations across the model parallel group.
    distribute_saved_activations: bool = False

    # The submodules to recompute. choices: "core_attn", "moe_act", "layernorm",
    # "mla_up_proj", "mlp", "moe", "shared_experts", "gdn_norm_out". default: ["core_attn"].
    # "core_attn": recompute the core attention part of the transformer layer. "moe_act":
    # recompute the MoE MLP activation function. "layernorm": recompute the input_layernorm
    # and pre_mlp_layernorm. "mla_up_proj": recompute the MLA up projection and RoPE applying
    # parts. "mlp": recompute the dense MLP submodule. "moe": recompute the MoE layer.
    # "shared_experts": recompute the shared experts in the MoE layer. "gdn_norm_out":
    # recompute the GatedDeltaNet output norm and HP-to-CP all-to-all. "moe_act", "layernorm",
    # "mla_up_proj", and "gdn_norm_out" use output-discarding checkpointing, "core_attn",
    # "mlp", "moe", and "shared_experts" use normal checkpointing.
    recompute_modules: list[str] | None | None = None
    ...
    # gradient tensors.
    fp8: Literal["e4m3", "hybrid"] | None | None = None
    ...
    fp8_quantizer_factory: str | None | None = None
    ...
    # If True, run the LM-head output projection with a TE ColumnParallelLinear under the
    # MXFP8 autocast context. Only active when fp8=True and fp8_recipe='mxfp8'.
    ...
    fp4: Literal["e2m1"] | None | None = None
    ...
    fp4_quantizer_factory: str | None | None = None

    # If True, the communication stream created by set_streams for combined 1f1b a2a overlap
    # is created with CUDA high priority.
    ...
    moe_shared_expert_intermediate_size: int | None | None = None
    ...
    # Use GroupedLinear(num_groups=1) for the shared expert MLP to trigger the Transformer
    # Engine grouped SwiGLU fusion path. Only effective when moe-shared-expert-intermediate-
    # size is set.
    use_grouped_gemm_for_shared_expert: bool = False

    # When set, GLU activations in the shared expert MLP will use a block interleaved format.
    # This is only effective when use_grouped_gemm_for_shared_expert is set.
    moe_shared_expert_glu_interleave_size: int | None | None = None

    # MoE Feed-Forward Network hidden size. If not specified, defaults to the ffn_hidden_size.
    moe_ffn_hidden_size: int | None | None = None
    ...
    moe_router_num_groups: int | None | None = None

    # Number of selected groups for group-limited routing.
    moe_router_group_topk: int | None | None = None
    ...
    moe_router_topk_scaling_factor: float | None | None = None
    ...
    moe_router_dtype: Literal["fp32", "fp64"] | None | None = None
    ...
    # `moe_router_load_balancing_type` is "quantile_balancing". At each global batch the bias
    # is updated as `qb_beta = ema * qb_beta + (1 - ema) * local_quantile`. The default 0.0
    # means no memory: the bias is replaced by the latest global-batch quantile estimate each
    # step.
    ...
    moe_router_force_biased: float | None | None = None

    # Use GroupedLinear(num_groups=1) for dense MLP to trigger the
    # ForwardGroupedMLP_CuTeGEMMSwiGLU_MXFP8 fusion on SM100+ with MXFP8 recipe. Requires
    # ``use_te_op_fuser=True`` and SwiGLU activation.
    ...
    # and ``use_transformer_engine_op_fuser=True``.
    ...
    moe_z_loss_coeff: float | None | None = None

    # Add noise to the input tensor by applying jitter with a specified epsilon value.
    moe_input_jitter_eps: float | None | None = None
    ...
    # Options are "deepep", "hybridep", and "ncclep". Currently only "hybridep" backend
    # supports the MNNVL case. "ncclep" uses NVIDIA NCCL Expert Parallelism via
    ...
    # Pad uneven HybridEP dispatch inputs to the group maximum before dispatch. Enable when
    # local HybridEP input token counts can differ across ranks, for example with dynamically
    # packed THD inputs. Leave disabled when dispatcher inputs are already padded to equal
    # token counts.
    ...
    moe_expert_capacity_factor: float | None | None = None
    ...
    moe_latent_size: int | None | None = None

    # Number of SMs for the flex token dispatcher's dispatch/combine communication, for all
    # backends (deepep, hybridep, ncclep). None lets each backend use its own default. Unifies
    # the deprecated per-backend moe_{deepep,hybridep}_num_sms knobs (routed in
    # __post_init__).
    moe_flex_dispatcher_num_sms: int | None | None = None
    ...
    moe_deepep_num_sms: int | None | None = None

    # DEPRECATED: use moe_flex_dispatcher_num_sms. Number of SMs to use for HybridEP (None
    # uses the default from DeepEP). If set, routed to moe_flex_dispatcher_num_sms in
    # __post_init__.
    moe_hybridep_num_sms: int | None | None = None

    # Number of CUDA thread blocks for the permute part in HybridEP. When
    # permute_fusion_into_hybridep is True, this sets the number of SMs for the permute part
    # (only 1 block per SM).
    moe_hybridep_num_blocks_permute: int | None | None = None

    # Number of CUDA thread blocks for the unpermute part in HybridEP. When
    # permute_fusion_into_hybridep is True, this sets the number of SMs for the unpermute part
    # (only 1 block per SM).
    moe_hybridep_num_blocks_unpermute: int | None | None = None
    ...
    # instead of narrowing to the (data-dependent) number of received tokens, removing the D2H
    # sync and dynamic shapes from the dispatch (required for CUDA-graph capture of the MoE
    # A2A and for the 1F1B EP comm overlap). The fused grouped GEMM consumes the ragged per-
    # expert counts on device and walks only the received tokens (no slack GEMM, no last-
    # expert padding). This requires the CuTe DSL / device-offset grouped GEMM, so it is only
    # supported with the fused op (use_transformer_engine_op_fuser,
    # NVTE_CUTEDSL_FUSED_GROUPED_MLP=1) on sm100+ (Blackwell or later); the dispatcher asserts
    # this. On older GPUs leave it False (dynamic shape). Defaults to False (narrow to the
    # received tokens).
    moe_ncclep_static_shape: bool = False

    # For the 'ncclep' flex dispatcher: use the NCCL symmetric-memory zero-copy IO path
    # (ep_bootstrap zero_copy + symm-mem-backed receive/combine buffers) instead of the
    # default HBM staged-copy path, saving one copy on the wire. Requires
    # moe_ncclep_static_shape and the fused op (use_transformer_engine_op_fuser). Defaults to
    # False.
    moe_ncclep_zero_copy: bool = False

    # When set, GLU activations in the MoE grouped MLP layer will use a block interleaved
    # format. Instead of interpreting the input tensor as a concatenation of gates and linear
    # units, it will be interpreted as alternating blocks of gates and linear units. This data
    # format is experimental and primarily intended to enable advanced fused kernels.
    moe_mlp_glu_interleave_size: int | None | None = None

    # moe_expert_rank_capacity_factor (float): The capacity factor for each expert rank, i.e.
    # the per-rank token budget. None means no token will be dropped. The default is None.
    ...
    # generously.
    moe_expert_rank_capacity_factor: float | None | None = None
    ...
    # iteration excluding optimizer) is enabled. cuda_graph_modules determines the scope of
    # graph capture.
    enable_cuda_graph: bool = False

    # Number of warmup steps for CUDA graphs. Note: GTP (``gtp_weight_remat_size > 1``) forces
    # a minimum of 2 per-graph warmup steps regardless of this value, because the first warmup
    # builds the weight-prefetch chain and the second exercises the prefetch path before
    # capture.
    ...
    # Determines the CUDA graph capture implementation. "none": no CUDA graph. "local": MCore
    # CUDA graph implementation. During training, graphable modules own per-layer CUDA graphs
    # controlled by cuda_graph_modules. During inference, graph ownership is controlled
    # separately by inference_cuda_graph_scope. "transformer_engine": Transformer Engine CUDA
    # graph implementation. During training, TE make_graphed_callables() creates per-layer
    # CUDA graphs controlled by cuda_graph_modules. Inference CUDA graphs are not supported;
    # inference_cuda_graph_scope must be "none". "full_iteration": full-iteration CUDA graph
    # implementation for the training iteration (1 CUDA graph for the whole forward-backward
    # path excluding the optimizer step). Inference CUDA graphs are not supported;
    # inference_cuda_graph_scope must be "none". cuda_graph_modules has no effect when
    # cuda_graph_impl="none" and must be empty when cuda_graph_impl="full_iteration".
    cuda_graph_impl: Literal["none", "local", "transformer_engine", "full_iteration"] = "none"

    # Controls the CUDA graph scope during inference. When unset, the effective default is
    # derived from cuda_graph_impl: "local" -> "layer", all other impls -> "none". "none":
    # inference runs in eager mode (no CUDA graphs). "layer": inference graphs are owned at
    # the module/layer boundary, e.g. TransformerLayer or MambaLayer. "block": inference
    # graphs are owned by the enclosing block, e.g. TransformerBlock or HybridBlock. Currently
    # supported combinations are: cuda_graph_impl="local" -> "layer" or "block"; all other
    # cuda_graph_impl values -> "none".
    inference_cuda_graph_scope: Literal["none", "layer", "block"] | None | None = None
    ...
    # symmetric memory.
    symmetric_ar_type: Literal["two_shot", "one_shot", "multimem_all_reduce"] | None | None = None
    ...
    # 'flashinfer': Uses FlashInfer cutlass_fused_moe. Not compatible with MXFP8. - 'torch':
    # Uses torch.nn.functional.grouped_mm (mcore_fused_moe with Triton kernels). Supports both
    # BF16 and MXFP8. - 'vllm': Uses vLLM's Triton fused MoE kernel (BF16). Avoids physical
    # token permutation via indirect addressing.
    ...
    # Token dispatcher to use for MoE expert parallelism during inference. - 'nccl':
    # AllGather/ReduceScatter via NCCL. Fixed token counts per rank; requires decode-only CUDA
    # graphs (forced automatically). - 'nvls': Variable-count AllGather-V/ReduceScatter-V via
    # NVLS multimem kernels. Requires Hopper+ GPUs with NVLink and symmetric memory. Default.
    ...
    # calculation.
    mrope_section: list[int] | None | None = None
    ...
    mamba_num_heads: int | None | None = None
    ...
    # "attn_proj", "mlp_norm", "expert_fc1", "moe_act", "fused_group_mlp". "attn_norm":
    # offload the input of the normalization in the attention part. "qkv_linear": offload the
    # input of the qkv linear part. "core_attn": offload the input of the core attention part.
    # "attn_proj": offload the input of the attn linear projection part. "mlp_norm": offload
    # the input of the normalization in the mlp part. "expert_fc1": offload the input of the
    # expert fc1 part. "moe_act": offload the input of the moe act part. "fused_group_mlp":
    # offload the input of the whole fused grouped MLP.
    ...
    # For more details, see the documentation: https://github.com/NVIDIA/Megatron-
    # LM/blob/main/docs/user-guide/features/fine_grained_activation_offloading.md#cuda-graph-
    # integration.
    delay_offload_until_cuda_graph: bool = False

    # Difference of offload bytes across PP ranks to balance the offload load. For more
    # details, see the documentation: https://github.com/NVIDIA/Megatron-
    # LM/blob/main/docs/user-guide/features/fine_grained_activation_offloading.md#tuning-
    # parameters.
    delta_offload_bytes_across_pp_ranks: int = 0

    # Fraction of eligible activation offload groups to offload across configured modules. For
    # details, see: https://github.com/NVIDIA/Megatron-LM/blob/main/docs/user-
    # guide/features/fine_grained_activation_offloading.md#activation-offload-fraction.
    ...
    # Scale factor for paged stash CUDA buffer allocation. Sign selects sizing: positive =
    # avg-based, negative = actual-max. Magnitude is headroom (e.g. 1.10 = 10%).
    moe_paged_stash_buffer_size_factor_cuda: float = 1.1

    # Scale factor for paged stash host buffer. 0 disables host buffer. Same sign convention
    # as moe_paged_stash_buffer_size_factor_cuda: positive = avg-based, negative = actual-max;
    # scale = abs(factor).
    moe_paged_stash_buffer_size_factor_cpu: float = 0.0

    # Per fine-grained offloading group name, max number of inflight offloads for that name
    # not yet joined on the main stream (wait_event on D2H). The same cap applies to every
    # name (e.g., ``moe_act`` and ``qkv_linear`` each have their own pending queue). 0 = wait
    # after every offload for that name. 1 = at most one not-yet-waited offload per name, etc.
    # None = do not insert these joins. This feature is particularly useful when using with
    # full-iteration CUDA graphs
    fine_grained_offloading_max_inflight_offloads: int | None | None = None

    # Number of encoder transformer layers.
    encoder_num_layers: int | None | None = None

    # Number of decoder transformer layers.
    decoder_num_layers: int | None | None = None

    # Use group-query attention.
    group_query_attention: bool = False

    # Window size for window attention. If not provided, window attention will be disabled.
    window_size: Any | None | None = None
    ...
    # attention.
    window_attn_skip_freq: Any | None | None = None

    # Maximum number of position embeddings to use. This is the size of position embedding.
    max_position_embeddings: int | None | None = None

    # Position embedding type.
    position_embedding_type: Literal[
        "learned_absolute", "rope", "yarn", "mrope", "relative", "none"
    ] = "learned_absolute"

    # Original maximum position embeddings for YaRN RoPE frequency correction.
    yarn_original_max_position_embeddings: int | None | None = None

    # Beta fast for YaRN RoPE frequency correction.
    yarn_beta_fast: float | None | None = None

    # Beta slow for YaRN RoPE frequency correction.
    yarn_beta_slow: float | None | None = None

    # Do not round YaRN correction range endpoints to integers.
    ...
    rotary_seq_len_interpolation_factor: int | None | None = None
    ...
    no_rope_freq: Any | None | None = None
    ...
    # Multiplier on --weight-decay for the qk-layernorm gains (1-D non-bias params whose name
    # contains q_layernorm. or k_layernorm.). These need a restoring force the most: attention
    # logits scale with gamma_q * gamma_k and nothing in the loss opposes their growth.
    # Default 0.0 = historical Megatron behaviour (no decay on 1-D params). Use 1.0 to decay
    ...
    # The final layernorm gain matters most, since it multiplies the LM head input directly.
    # Default 0.0 = no decay. Use 1.0 to decay like every other weight (as OLMo 2/3 do), or a
    ...
    no_weight_decay_cond_type: Literal["apply_wd_to_qk_layernorm"] | None | None = None
    ...
    micro_batch_size: int | None | None = None
    ...
    global_batch_size: int | None | None = None
    ...
    # instead.
    rampup_batch_size: list[int] | None | None = None
    ...
    step_batch_size_schedule: str | None | None = None
    ...
    # hashes not checked.
    check_weight_hash_across_dp_replicas_interval: int | None | None = None

    # Interval (in iterations) to run GPU performance sniff tests (GEMMs, all-to-all,
    # send/recv). Flags any rank whose throughput differs from the mean by more than one
    # standard deviation. If not specified, sniff tests are not run.
    gpu_sniff_test_interval: int | None | None = None

    # Training CPU-GPU synchronization interval, to ensure that CPU is not running too far
    # ahead of GPU.
    train_sync_interval: int | None | None = None
    ...
    train_samples: int | None | None = None

    # Exit the program after the iteration is divisible by this value.
    exit_interval: int | None | None = None

    # Exit the program after this many minutes.
    exit_duration_in_mins: int | None | None = None
    ...
    result_rejected_tracker_filename: str | None | None = None

    # Config file when tp_comm_overlap is enabled.
    tp_comm_overlap_cfg: str | None | None = None
    ...
    # rope by default.
    rope_type: Literal["rope", "yarn"] | None | None = None
    ...
    dump_param_to_param_group_map: str | None | None = None
    ...
    dataloader_type: Literal["single", "cyclic", "external"] | None | None = None
    ...
    # Number of trainer batches of rollout generation lag to allow. The number of in-flight
    # trainer batches is this value plus one. Requires --rl-partial-rollouts when greater than
    # 0.
    rl_generation_lag: int = 0

    # Granularity for submitting rollout generation work. R submits individual rollouts
    # independently while still yielding complete rollout groups to training. G submits one
    # rollout group at a time. B submits grpo_prompts_per_step rollout groups together.
    rl_submission_granularity: Literal["R", "G", "B"] = "B"

    # Granularity for consuming generated rollout groups. G consumes groups as they complete.
    # B consumes complete trainer batches in submission order. R is not currently supported.
    rl_consumption_granularity: Literal["R", "G", "B"] = "B"
    ...
    langrl_env_config: str | None | None = None
    ...
    # deleted on suspend and re-captured on resume.
    ...
    # version of the policy. Use --rl-generation-lag to control the degree of staleness.
    ...
    rl_importance_sampling_truncation_coef: float | None | None = None
    ...
    rl_inference_tensor_model_parallel_size: int | None | None = None

    # Degree of pipeline model parallelism for inference for RL.
    rl_inference_pipeline_model_parallel_size: int | None | None = None

    # Degree of expert model parallelism for inference for RL.
    rl_inference_expert_model_parallel_size: int | None | None = None
    ...
    rl_inference_expert_tensor_model_parallel_size: int | None | None = None
    ...
    # (when UVM is not enabled; requires torch_memory_saver to be installed).
    ...
    rl_profile_dir: str | None | None = None
    ...
    start_weight_decay: float | None | None = None

    # End of run weight decay coefficient for L2 regularization.
    end_weight_decay: float | None | None = None
    ...
    non_persistent_save_interval: int | None | None = None
    ...
    non_persistent_global_ckpt_dir: str | None | None = None

    # Directory containing local non-persistent model checkpoints.
    non_persistent_local_ckpt_dir: str | None | None = None
    ...
    pretrained_checkpoint: str | None | None = None

    # Checkpoint step to load model from.
    ckpt_step: int | None | None = None
    ...
    ckpt_convert_format: Literal["torch", "torch_dist"] | None | None = None

    # Save directory for converted checkpoint.
    ckpt_convert_save: str | None | None = None
    ...
    # ranks.
    replication_jump: int | None | None = None

    # Number of machines storing the replica of a given rank's data.
    replication_factor: int = 2

    # Whether to hash checkpointing files during save and validate their integrity during
    # load.
    verify_integrity: bool = False

    # Per-tensor streaming dequantize when loading checkpoints with quantized model params
    # (FP8, MXFP8, blockwise FP8, NVFP4). The LoadPlanner dequantizes one destination at a
    # time, instead of dequantizing the entire state dict to high precision before the load
    # starts (which allocates N simultaneous scratch tensors and can OOM on large models). On
    # by default; pass --no-stream-ckpt-dequant to fall back to the legacy upfront pass.
    stream_ckpt_dequant: bool = True

    # Do not save current optimizer.
    no_save_optim: bool | None | None = None

    # Do not save current rng state.
    no_save_rng: bool | None | None = None

    # Do not load optimizer when loading checkpoint.
    no_load_optim: bool | None | None = None

    # Do not load rng state when loading checkpoint.
    no_load_rng: bool | None | None = None

    # Override the iteration stored in the loaded checkpoint. Also resets
    # consumed_train_samples accordingly so the data loader replays samples from that
    # iteration onward.
    override_ckpt_iteration: int | None | None = None

    # Deprecated: see --ckpt-format.
    use_dist_ckpt_deprecated: bool = False

    # Deprecated: see --ckpt-format.
    dist_ckpt_format_deprecated: Any | None | None = None
    ...
    loss_scale: float | None | None = None
    ...
    # Dtype of the materialized inter-chunk SSM states in Mamba training
    mamba_training_ssm_states_dtype: Literal["fp32", "bf16"] | None | None = None
    ...
    nccl_communicator_config_path: str | None | None = None
    ...
    sharp_enabled_group: Literal["dp", "dp_replica"] | None | None = None
    ...
    distributed_timeout_seconds_after_init: int | None | None = None
    ...
    flight_recorder_dump_path: str | None | None = None
    ...
    decoder_first_pipeline_num_layers: int | None | None = None
    ...
    decoder_last_pipeline_num_layers: int | None | None = None
    ...
    pipeline_model_parallel_layout: str | None | None = None

    # Old model parallel argument, do not use. Use --tensor-model-parallel-size instead.
    model_parallel_size: int | None | None = None

    # Number of layers per virtual pipeline stage
    num_layers_per_virtual_pipeline_stage: int | None | None = None

    # Number of virtual pipeline stages per pipeline parallelism rank
    num_virtual_stages_per_pipeline_rank: int | None | None = None
    ...
    ddp_num_buckets: int | None | None = None

    # Bucket size for data-parallel communication
    ddp_bucket_size: int | None | None = None
    ...
    ddp_param_name_patterns_for_fp32_local_accumulation: list[str] = field(
    ...
    # Opt out of the precomputed LayerWise param layout. When set, falls back to the legacy
    # LayerWise ping-pong path: all params (including non-Muon embeddings, biases, layernorm)
    # live in a single LayerWise buffer and the optimizer uses the allgather_params()
    # codepath. The default (precomputed layout) routes non-Muon params through a separate
    # DistributedOptimizer with byte-level sharding, which is faster and uses less padding but
    # produces different bf16 reduction ordering and so will not match legacy-path loss curves
    # bit-for-bit.
    ...
    suggested_communication_unit_size: int | None | None = None
    ...
    # run during training.
    eval_interval: int | None | None = None
    ...
    start_eval_at_iter: int | None | None = None
    ...
    eval_global_batch_size: int | None | None = None
    ...
    # per eval step.
    eval_micro_batch_size: int | None | None = None
    ...
    # Optional list of names for multiple validation sets. When provided with --multiple-
    # validation-sets, these names are used instead of numeric indices (e.g. 'validation-wiki'
    # instead of 'validation-0'). The number of names must match the number of validation
    # datasets.
    validation_set_names: list[str] | None | None = None
    ...
    # big.
    data_args_path: str | None | None = None
    ...
    # `test` keys
    per_split_data_args_path: str | None | None = None
    ...
    per_dataset_sequences_path: Any | None | None = None
    ...
    data_cache_path: Any | None | None = None
    ...
    dataloader_prefetch_factor: int | None | None = None
    ...
    # Return cu_seqlens marking document boundaries within each sample so that attention is
    # restricted to individual documents. Note that --reset-attention-mask does NOT achieve
    # this: the GPT layer specs pin attn_mask_type to causal and TE only reads attention_mask
    # for padding and arbitrary mask types, so the dataloader mask is silently discarded. This
    # also restarts RoPE at every document boundary, making --reset-position-ids redundant.
    dataloader_inter_document_masking: bool = False

    # With --dataloader-inter-document-masking, print this rank's cu_seqlens for the first N
    # calls to get_batch. cu_seqlens is a few dozen bytes, so logging it is far cheaper than
    # any runtime cross-check -- and unlike a collective it cannot deadlock against the
    # pipeline p2p chain. Every rank sharing a data-parallel index must print identical values
    # for a given call index; verify with scripts/korbi/check_cu_seqlens_agreement.py. DEBUG
    # ONLY: the readback it needs is the per-microbatch stream drain the scatter exists to
    # remove. OELLM patch, see megatron/training/packed_doc_attention.py.
    packed_doc_attention_log_cu_seqlens: int = 0
    ...
    object_storage_cache_path: str | None | None = None
    ...
    padded_vocab_size: int | None | None = None

    # Whether to pad vocab size of the model automatically if padded_vocab_size is not
    # provided.
    pad_vocab_size: bool = True

    # Path to the vocab file.
    vocab_file: str | None | None = None

    # Path to the BPE merge file.
    merge_file: str | None | None = None
    ...
    tokenizer_model: str | None | None = None
    ...
    tiktoken_pattern: Literal["v1", "v2"] | None | None = None
    ...
    null_tokenizer_eod_id: int | None | None = None

    # Pad token id for NullTokenizer. Defaults to -1 (no pad token). Set to a value outside
    # the dataset to avoid masking real tokens.
    null_tokenizer_pad_id: int = -1

    # Custom chat template in jinja format for conversation formatting.
    chat_template: str | None | None = None
    ...
    ict_head_size: int | None | None = None
    ...
    evidence_data_path: str | None | None = None
    ...
    block_data_path: str | None | None = None

    # Where to save/load Open-Retrieval Embedding data to/from
    embedding_path: str | None | None = None
    ...
    num_experts: int | None | None = None
    ...
    # sample; "sinkhorn" corresponds to the balancing algorithm used in S-BASE;
    # "quantile_balancing" (QB) uses dual coordinate descent on a per-expert bias to handle
    # load balance internally; "none" implies no load balancing. The default is "aux_loss".
    moe_router_load_balancing_type: Literal[
        "aux_loss", "seq_aux_loss", "global_aux_loss", "sinkhorn", "quantile_balancing", "none"
    ] = "aux_loss"
    ...
    q_lora_rank: int | None | None = None
    ...
    linear_attention_freq: Any | None | None = None
    ...
    heterogeneous_layers_config_path: str | None | None = None
    ...
    heterogeneous_layers_config_encoded_json: str | None | None = None
    ...
    tensorboard_dir: str | None | None = None
    ...
    log_memory_interval: int | None | None = None
    ...
    wandb_project: str | None | None = None

    # The wandb experiment name.
    wandb_exp_name: str | None | None = None

    # Path to save the wandb results locally.
    wandb_save_dir: str | None | None = None
    ...
    # Directory for MoE router decision traces (JSONL). When set, a RouterTracer is
    # initialized at training start and hooks are registered on all TopKRouter modules. Traces
    # are written in the same format as inference traces so the analysis scripts under
    # tools/moe_routing work on both.
    moe_routing_trace_path: str | None | None = None

    # Maximum number of training iterations to trace. Tracing stops automatically after this
    # many calls to advance_step(). Defaults to tracing all iterations when
    # moe_routing_trace_path is set. (Inference uses --moe-routing-trace-max-inference-steps
    # instead.)
    moe_routing_trace_max_training_iters: int | None | None = None
    ...
    # Log mean/std/min/max of every RMSNorm/LayerNorm gain, per layer. Norm gains get
    # wd_mult=0 by default, so nothing opposes their drift; if they do go extreme the levers
    # are --residual-norm-wd-mult / --qk-layernorm-wd-mult or zero-centred RMSNorm.
    ...
    # Top-P (nucleus) threshold applied after top-K selection when saving logits. Only the
    # smallest set of entries whose cumulative probability mass reaches this threshold is
    # kept. Must be in (0, 1].
    logits_save_top_p: float | None | None = None

    # Minimum number of entries kept per token when top-P masking is active, regardless of
    # cumulative mass. Default: 1.
    logits_save_top_p_min_k: int = 1

    # Directory to save logits.
    logits_save_dir: str | None | None = None

    # Dtype for on-disk top-K log-probabilities.
    logits_save_dtype: Literal["fp16", "bf16", "fp32"] = "fp16"

    # Directory to load logits.
    logits_load_dir: str | None | None = None

    # Number of decode threads for cached-logits zstd decompression and torch.load processing.
    logits_load_decode_threads: int = 4

    # PyTorch DataLoader prefetch factor for decoded cached-logits iterations. (Non-MSC only)
    logits_load_prefetch_factor: int = 3

    # For MSC/object-storage logits tar shards, number of whole tar shards to prefetch into
    # the MSC cache ahead of sequential tar consumption.
    logits_load_msc_prefetch_depth: int = 2

    # KD loss alpha for loading logits. Total loss is calculated as alpha * kd_loss + (1 -
    # alpha) * lm_loss.
    logits_load_kd_loss_alpha: float = 1.0

    # When set, KD loss errors are logged as warnings and training falls back to LM-only loss
    # instead of crashing.
    ...
    # ==SUPPRESS==
    cuda_graph_scope_deprecated: list[Any] | None | None = None

    # Selects training capture coverage within per-layer CUDA graphs (local and
    # transformer_engine implementations). Valid values are "attn", "mlp", "moe",
    # "moe_router", "moe_preprocess", and "mamba": "attn": captures operations in
    ...
    # mamba layer. An empty list means capturing the whole Transformer layer. This field is
    # meaningless when --cuda-graph-impl=full_iteration and must be empty. Backward
    # compatibility: "full" is deprecated but kept for backward compatibility; it is
    # transformed to an empty list in validate_args. The deprecated values "full_iteration"
    # and "full_iteration_inference" are also accepted and migrated to the new API in
    # validate_args.
    cuda_graph_modules: list[Any] = field(default_factory=lambda: [])
    ...
    inference_dynamic_batching_paused_buffer_size_gb: float | None | None = None
    ...
    inference_dynamic_batching_mamba_memory_ratio: float | None | None = None
    ...
    inference_dynamic_batching_max_requests: int | None | None = None

    # Override the inference context's default `max_tokens`.
    inference_dynamic_batching_max_tokens: int | None | None = None
    ...
    # Token ceiling for the largest captured prefill/mixed CUDA graph (default: 512). Clamped
    # to at least the decode limit `max_requests * (num_speculative_tokens + 1)` and at most
    # `max_tokens`. Ignored when --inference-cuda-graph-all-prefills is set (which extends
    # capture to the full `max_tokens`).
    ...
    # blocks cannot be shared between requests with identical prompt prefixes.
    ...
    # Coordinator routing policy for prefix caching. "load_balanced" (default) routes to the
    # rank with the fewest in-flight requests, ignoring prefix affinity. "first_prefix_block"
    # routes based on the first block hash only. "longest_prefix" routes to the rank with the
    # longest matching prefix. "first_prefix_block" and "longest_prefix" both combine prefix
    # affinity with load balancing and fall back to load-balanced routing when prefix caching
    # is disabled or no prefix match exists.
    ...
    # models. When set, Mamba states at block boundaries are cached for reuse. This budget
    # covers both the durable cache (the ssm_states/conv_states slots reused across requests)
    # and the per-step extraction scratch (the intermediate_ssm_out/intermediate_conv_out
    # buffers, sized to min(ceil(max_tokens / block_size), 3 * max_requests) slots); the
    # scratch is reserved first, so a smaller max_tokens (or max_requests) shrinks the scratch
    # and leaves more durable slots.
    inference_dynamic_batching_prefix_caching_mamba_gb: float | None | None = None
    ...
    # Spacing of CUDA graph token counts. "exponential" (default) halves from
    # cuda_graph_max_tokens down to tp_size, giving a log-spaced distribution with bounded
    # relative padding. "linear" uses varying linear strides across the range.
    ...
    # Use the same inference sampling seed on every data-parallel rank. --deterministic-mode
    # also uses the same seed on every DP rank.
    offset_sampling_seed_by_dp_rank: bool = True

    # Async scheduling mode for dynamic batching. "legacy" (default) preserves the existing
    # resolve-before-prepare path. "async" overlaps asynchronous scheduling phases by
    # reordering them to prepare-before-resolve.
    inference_dynamic_batching_async_sched_mode: Literal["legacy", "async"] = "legacy"

    # How returned inference log-probs are computed engine-wide. "raw_logprobs" (default) uses
    # the unmodified model logits; "processed_logprobs" uses temperature and filters by
    # top-k/top-p.
    ...
    inference_coordinator_port: int | None | None = None
    ...
    # Skip the EP-group consensus all-reduce in the inference engine control loop and step on
    # local state only. Pause/unpause take effect as soon as the signal is delivered to a
    # rank. Only safe when EP coordination is not required (e.g. ep_world_size == 1).
    inference_disable_ep_consensus: bool = False

    # Partition the world into independent inference models, each with its own parallelism,
    # e.g. "tp=2,role=prefill+tp=1,dp=2,role=decode". Shards are separated by "+" or ";"; per-
    # shard keys are tp,pp,ep,expt_tp,dp (each defaults to 1) and must partition the full
    # world. Tagging shards role=prefill|decode enables disaggregated inference (prefill hands
    # KV to the decode pool); a dp>1 decode shard is several independent decode instances.
    inference_shards: str | None | None = None
    ...
    te_precision_config_file: Any | None | None = None
    ...
    # will be used for the main weight data-type. Replaces --main-params-dtype.
    ...
    # main gradient / accumulation data-type. Replaces --main-grads-dtype.
    ...
    # If set, enables fine-grained parameter gathering for Megatron-FSDP. This allows greater
    # overlap between parameter all-gather operations and forward computation, at the cost of
    # additional communication calls. For MXFP8, this helps save memory during fine-grained
    # activation recomputation, because MXFP8 forward and backward passes use different
    # parameter representations (rowwise data for forward, colwise data for backward). Only
    # the rowwise parameters of modules involved in recomputation will be unsharded.
    megatron_fsdp_enable_fine_grained_param_gather: bool = False

    # When using Megatron-FSDP double buffering, use the MaxPoolAllocator instead of the
    # FixedPoolAllocator to support asymmetrical FSDP unit configurations. Will increase
    # memory overhead to recycle buffers that fit all FSDP units. Enables NCCL user buffer
    # registration and CUDA graph replay for mixed-arch models.
    megatron_fsdp_max_pool_double_buffer: bool = False

    # When using Megatron-FSDP double buffering, persist non-unit modules that are not
    # included in the symmetric buffer pool. May be necessary for NCCL UBR or CUDA Graphs on
    ...
    one_logger_run_name: str | None | None = None
    ...
    app_tag_run_name: str | None | None = None
    ...
    inprocess_max_iterations: int | None | None = None
    ...
    # Enable the usage of Multi-Storage Client (MSC) in Megatron Core. Disabled by default;
    # pass this flag to opt in.
    enable_msc: bool = False

    # [DEPRECATED] MSC is disabled by default; this flag is a no-op and will be removed in a
    # future release.
    disable_msc_deprecated: bool = False
    ...
    fault_injector_ranks: str | None | None = None

    # Number of ranks to inject faults on (random selection).
    fault_injector_num_ranks: int | None | None = None

    # Comma-separated list of fault types to inject (e.g. 'hang,crash').
    fault_injector_fault_types: str | None | None = None

    # Comma-separated list of fault probabilities (normalized at runtime).
    fault_injector_fault_probabilities: str | None | None = None

    # Force a specific fault delay in seconds from training start or delay_start_iteration.
    fault_injector_fault_delay: float | None | None = None

    # Start the fault delay timer after iteration N completes. If unset, fault delay timing
    # starts from the beginning of training.
    fault_injector_delay_start_iteration: int | None | None = None

    # Mean time to inject (MTTI) in seconds; used when fault_delay is None.
    fault_injector_mtti_seconds: float | None | None = None

    # Offset seconds added to the sampled fault delay.
    fault_injector_offset_seconds: float | None | None = None

    # RNG seed for the fault injector.
    fault_injector_seed: int | None | None = None
