"""templates/jupiter_relay.sbatch: the worker-only SIGTERM relay renders to
valid bash with the trap, the background main step and the wait loop, and its
pkill pattern hits torchrun's workers but not the agent."""
...
REPL = dict(
    sbatch_directives="#SBATCH --nodes=4\n#SBATCH --signal=B:TERM@240",
    env_exports="export X=1",
    job_name="t",
    srun_opts="--exclusive ",
    launcher_cmd="apptainer exec img.sif",
    command="python -m torch.distributed.run pretrain_gpt.py --a 1",
)
...
def test_relay_targets_direct_children_of_the_agents():
    ...
    pat = m.group(1)
    assert re.search(
        pat, "/opt/venv/bin/python  -u -m torch.distributed.run --nnodes 4 pretrain_gpt.py"
    )
    assert re.search(pat, "python3 -m torch.distributed.run pretrain_gpt.py")
    assert (
        re.search(
            pat,
            "bash -c export X=1; apptainer exec img.sif python -u -m torch.distributed.run pretrain_gpt.py",
        )
        is None
    )
    assert (
        re.search(pat, "apptainer exec img.sif python -u -m torch.distributed.run pretrain_gpt.py")
        is None
    )
