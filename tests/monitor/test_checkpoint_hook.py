"""Checkpoint hook (2026-09-06): the `checkpoint_saved` log event runs the
job's checkpoint_hook_command when a persistent checkpoint's iteration is a
multiple of checkpoint_hook_every (IterationMultipleCondition)."""
...
POLICIES = [
    Path(__file__).resolve().parents[2] / "config" / "job" / f
    for f in ("auto_restart_ckptreset_faultscan.yaml", "auto_restart_ckptreset.yaml")
]
...
def test_pattern_matches_persistent_saves_only():
    ...
    assert (
        m
        and m.group("iteration") == "80000"
        and m.group("ckpt_dir").endswith("/training_ckpts/checkpoints")
    )
...
def test_log_event_extracts_the_groups():
    ...
    cfg = LogEventConfig(
        ...
        pattern=e["pattern"],
        pattern_type="regex",
        extract_groups=e["extract_groups"],
        action=RunCommandActionConfig(command="x"),
    )
    trig = LogEvent(cfg).check_triggers(PERSISTENT + "\n" + ROLLING)
    assert (
        len(trig) == 1
        and trig[0]["iteration"] == "80000"
        and trig[0]["ckpt_dir"].endswith("/checkpoints")
    )
