...
POLICY = Path(__file__).resolve().parents[2] / "config" / "job" / "auto_restart_ckptreset_faultscan.yaml"
...
SIGNAL_STEP_LINE = "slurmstepd: error: *** STEP 1691075.0 ON jpbo-008-04 CANCELLED AT 2026-09-06T15:39:18 ***"
...
SCANCEL_STEP_LINE = "slurmstepd: error: *** STEP 1486215.0 ON jpbo-002-01 CANCELLED AT 2026-08-25T08:21:49 ***"
TIME_LIMIT_LINE = "slurmstepd: error: *** JOB 1537344 ON jpbo-002-01 CANCELLED AT 2026-08-30T22:59:07 DUE TO TIME LIMIT ***"
SIGTERM_EXIT_LINE = "[default0]:[exiting program after receiving SIGTERM.] datetime: 2026-09-06 17:57:10 "
DURATION_EXIT_LINE = "[default0]:[exiting program after 690.0159624854724 minutes] datetime: 2026-08-25 19:54:10 "


def load_rules():
    cfg = yaml.safe_load(POLICY.read_text())
    return [e for e in cfg["log_events"] if e.get("pattern") is not None or e.get("pattern_type") in ("inactivity", "progress")]
...
def first_terminal(text):
    """Name of the first rule (in policy order) whose pattern matches ``text`` and whose action is
    terminal (Cancel/Finish/Restart), i.e. what _process_log_events returns on this log excerpt."""
    for e in load_rules():
        ...
        if cls not in ("CancelAction", "FinishAction", "RestartAction"):
            continue
        cfg = LogEventConfig(name=e["name"], pattern=e["pattern"], pattern_type=e.get("pattern_type", "substring"),
...
def test_rules_present_in_the_right_order():
    ...
    i = {n: names.index(n) for n in ("operator_cancel", "sigterm_rollover", "time_limit_rollover", "exit_duration_rollover")}
    assert i["operator_cancel"] < i["sigterm_rollover"] < i["time_limit_rollover"] < i["exit_duration_rollover"]
...
def test_operator_cancel_pattern():
    ...
    assert re.search(pat, TIME_LIMIT_LINE) is None, "the wall-limit line must not read as an operator cancel"
    assert re.search(pat, SCANCEL_STEP_LINE) is None, "only the JOB line is needed; STEP lines follow it"
