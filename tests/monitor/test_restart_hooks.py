    ...
    with initialize_config_dir(config_dir=str(config_dir), version_base=None):
        cfg = compose(
            ...
            overrides=[
                ...
                "++job.restart_pre_command=scan-the-log",
                "++job.restart_exclude_file=/tmp/list",
            ],
        )
...
def test_restart_hook_variables_include_log_dir(tmp_path):
    """The pre_command may scan the run's whole log directory ({log_dir}), not
    only the failed job's log:

    the lines that name a faulty node are written after the kill
    (2026-09-06, jobs 1692960/1693057).
    """
    ...
    job = JobRecord(
        ...
        runtime=JobRuntime(submitted=True, runtime_job_id="7"),
    )
    loop._pending_restart_hooks = {
