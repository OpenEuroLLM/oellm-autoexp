...
def visualize_plan(
    ...
    """Visualize the execution plan in ASCII format."""

    # Group jobs by stage
    stages_dict = group_jobs_by_stage(jobs)

    # Sort stages (stable first, then others alphabetically)
    stage_order = sorted(stages_dict.keys(), key=lambda s: (s != "stable", s))
    ...
    for stage_idx, stage in enumerate(stage_order):
        ...
        if stage == "stable" or stage == "unknown":
            print("│ Start: Immediate".ljust(69) + "│")
        else:
            if sample_job.start_conditions:
