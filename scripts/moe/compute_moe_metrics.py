...
def _build_megatron_indexed_batches(
    ...
    for src_idx, (weight, prefix) in enumerate(sources):
        ...
        for doc_id in order:
            ...
            if n == 0:
                continue
            # Do not let a long document consume the following source's token
            # budget; each source must remain proportional to its blend weight.
            end = min(cursor + n, src_end, buf.size)
            n = end - cursor
            buf[cursor:end] = tokens[:n]
            cursor = end
            # Insert EOD between documents to mirror Megatron training behavior.
            if cursor < src_end and cursor < buf.size:
...
def main() -> None:
    ...
    if use_random or use_megatron_indexed:
        ...
        tokenizer = None
        # Exclude document-separator/padded EOD tokens from router metrics. For
        # openeurollm-256k this is 2; the CLI keeps other tokenizers configurable.
        pad_id = (
            args.moe_eod_token
            if use_megatron_indexed and args.moe_eod_token is not None
            else 0
        )
    else:
