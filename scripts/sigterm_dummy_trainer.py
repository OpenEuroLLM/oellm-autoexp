#!/usr/bin/env python3
"""Dummy trainer for the segment-end test of the monitor policy (1 node, no
GPU, stdlib only).
...
def main():
    ...
    if os.path.exists(tracker):
        ...
        with open(os.path.join(a.ckpt_dir, f"iter_{start:07d}", "state.json")) as fh:
            st = json.load(fh)
        say(
            f"successfully loaded checkpoint from {a.ckpt_dir} [ t 0, p 0 ] at iteration {start} (saved by job {st.get('job')} at {st.get('saved_at')})"
        )
    else:
        say(f"no checkpoint under {a.ckpt_dir}: starting at iteration 0")
    say(
        f"job {os.environ.get('SLURM_JOB_ID')} on {os.uname().nodename}: iterations {start + 1}..{a.iters}, {a.tick}s each, persistent save every {a.save_every}"
    )
    it = start
    while it < a.iters:
        it += 1
        time.sleep(a.tick)
        say(
            f"iteration {it:8d}/{a.iters:8d} | consumed samples: {it * 4096:12d} | elapsed time per iteration (ms): {a.tick * 1000:.1f} | lm loss: 1.500000E+00 | grad norm: 0.500"
        )
        if _received:
